                        ...
                        const std::string &section_name)
{
    ...
    if (sprintf(path, package_path, package_name.c_str()) <= 0) {
                     ...
                     const std::string &section_name)
{
    ...
    if (sprintf(sec_path, section_type_path, package_name.c_str(), section_name.c_str(),
                section_type.c_str()) <= 0) {
        LOG(ERROR) << "Failed to compose path";
        return false;
    }
                     ...
                     const std::unordered_map<std::string, std::string> &options)
{
    ...
    for (auto &option : options) {
        ...
        if (sprintf(opt_path, option_path, package_name.c_str(), section_name.c_str(),
                    option.first.c_str()) <= 0) {
            LOG(ERROR) << "Failed to compose path";
            return false;
        }
    ...
    if (sprintf(pkg_path, package_path, package_name.c_str()) <= 0) {
        LOG(ERROR) << "Failed to compose path";
        return false;
    }
                     ...
                     std::unordered_map<std::string, std::string> &options)
{
    ...
    char path[MAX_UCI_BUF_LEN] = {0};
    if (sprintf(path, section_path, package_name.c_str(), section_name.c_str()) <= 0) {
        LOG(ERROR) << "Failed to compose path";
        return false;
    }
                    ...
                    std::string &option_value)
{
    ...
    char path[MAX_UCI_BUF_LEN] = {0};
    if (sprintf(path, option_path, package_name.c_str(), section_name.c_str(),
                option_name.c_str()) <= 0) {
        LOG(ERROR) << "Failed to compose path";
        return false;
    }
                        ...
                        const std::string &section_name)
{
    ...
    char sec_path[MAX_UCI_BUF_LEN] = {0};
    if (sprintf(sec_path, section_path, package_name.c_str(), section_name.c_str()) <= 0) {
        LOG(ERROR) << "Failed to compose path";
        return false;
    }
